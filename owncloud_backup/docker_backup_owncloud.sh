...
# Back up owncloud MySQL database
docker exec ${CONTAINER_NAME_MYSQL} /usr/bin/mysqldump -u ${MYSQL_USERNAME} --password=${MYSQL_PASSWORD} owncloud > ${PATH_TO_BACKUPS_MYSQL}/mysql_$(date +%Y%m%d_%H-%M-%S).sql
